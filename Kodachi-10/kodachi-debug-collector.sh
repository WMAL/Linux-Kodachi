...
#
# Measured 2026-09-06 on an installed Kodachi VM (claude-ff7aea05): the served
...
show_menu() {
    ...
    echo -e "${GREEN}"
    echo "╔═══════════════════════════════════════════════════════════╗"
    echo "║         KODACHI OS DEBUG COLLECTOR v1.7                  ║"
    echo "╚═══════════════════════════════════════════════════════════╝"
    echo -e "${NC}"
    echo ""
...
for hooks_pattern in "/opt/kodachi/dashboard/hooks" "${REAL_HOME}/dashboard/hooks" "/opt/*/dashboard/hooks"; do
    for hooks_dir in $hooks_pattern; do
        if [[ -d "$hooks_dir/config" ]]; then
            ...
            # 2026-09-06 on the live ISO 10.0.1 build 8: `--all` sat at step
...
if [[ -n "$KDC_HOOKS" ]]; then
    ...
    # on disk after a partial update. GUI/session binaries are not asked for
    # --version (that would try to open a window as root).
    _ph0=$(date +%s.%N)
    {
        ...
        for _b in "$KDC_HOOKS"/*; do
            ...
            case "$_n" in
                kodachi-dashboard|kodachi-session-helper|tun2socks*) continue ;;
            esac
